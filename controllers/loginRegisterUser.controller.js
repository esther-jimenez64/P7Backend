...
exports.signup = (req, res) => {
  ...
  bcrypt.hash(req.body.password, 10) //Nous appelons la fonction de hachage de bcrypt dans notre mot de passe et lui demandons de saler le mot de passe 10 fois.//
    ...
    .then(hash => {
      const utilisateur = {    //Création d'un objet contenant l'user créé par l'utilisateur contenue dans la requête//
      ...
      if (req.body.email != findEmail){  //si l'email communiqué est différent que celui de la database//   
...
exports.login = (req, res,) => { 
  ...
  const findEmail = USER.findOne({ where: { email: email } })//Nous récupérons un user qui à le même email que l'email dans la database/
    .then(user => {
      if (!user) { //si ce n'est pas le cas//
        console.log('User Not found!');
        res.status(401).json();
      }
      var passwordAuth = req.body.password;
      if (user) { //si l'email et bien dans la database//
      bcrypt.compare(passwordAuth, user.password) //Nous utilisons la fonction compare de bcrypt pour comparer le mot de passe entré par l'utilisateur avec le hash enregistré dans la base de données.//
        .then(valid => {
          if (!valid) {
