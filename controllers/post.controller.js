...
exports.createPost = (req, res) => {
  ...
  if (req.file) {
    //vérifier si fichier image existe ou non S'il existe, on traite la nouvelle image//
    const photo = `${req.protocol}://${req.get("host")}/images/${
      req.file.filename
    }`; //Reconstruire l'URL complète du fichier enregistré//
    const publication = {
      //Création d'un objet contenant le post créé par l'utilisateur contenue dans la requête//
      userId: userId,
    ...
    //Vérifier si fichier image  n'existe pas dans cas là, nous n'envoyons pas d'image//
    const publication = {
      userId: userId,
      title: req.body.title,
      content: req.body.content,
...
exports.findAllPublication = (req, res) => {
  return POST.findAll({
    //Nous récupérons les posts avec les comment en ordre du dernier publier//
    include: [db.comments],
    order: [["id", "DESC"]],
  })
    .then((cities) => {
...
exports.modifyPost = (req, res) => {
  ...
  POST.findByPk(id) //Nous récupérons le post qui  contient cette id//
    .then((postModif) => {
      if (postModif.userId !== req.auth.userId && req.auth.isAdmin !== true) {
        //Si l'user Id et n'est pas le même que celui de la requête ou que l'admin et = false//
        return res.status(401).json({ error: "Unauthorized request" });
      } else {
        if (postModif.image == null) {
          //si fichier image  n'existe pas dans cas là, nous n'envoyons la nouvelle d'image//
          if (req.file) {
            /*si l'user a envoyer une image*/
            const photo = `${req.protocol}://${req.get("host")}/images/${
              req.file.filename
            }`; //Reconstruire l'URL complète du fichier enregistré.//
            const publication = {
              //Création d'un objet contenant le post modifié par l'utilisateur contenue dans la requête//
            ...
            POST.update(publication, {
              //Ensuite, nous créons le post à envoyer à la base de donnée/
              where: { id: id },
            });
            res.send({
            ...
            photo = null; /*si l'user n'a pas envoyer une photo*/
            const publication = {
              //Création d'un objet contenant le post modifié par l'utilisateur contenue dans la requête//
              userId: req.auth.userId,
              title: req.body.title,
...
exports.deletePost = (req, res) => {
  var id =
    req.params.id; /*Récupération de l'id du comment contenue dans params URL*/
  POST.findByPk(
    id
