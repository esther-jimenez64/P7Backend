...
exports.createComment = (req, res) => {
  ...
  const postId =
    req.params
      .PostId; /*Nous récupérant l'id du post contenue dans les params Url*/

  const commentario = {
    //Création d'un objet contenant le commentaire créé par l'utilisateur contenue dans la requête//
    userId: userId,
...
exports.modifyComment = (req, res) => {
  var id =
    req.params
      .CommentId; /*Récupération de l'id du comment contenue dans params URL*/
  Comment.findByPk(
    id
    ...
    .then((commentModif) => {
      ...
      if (commentModif.userId !== req.auth.userId && isAdmin !== true) {
        // ID utilisateur de la demande, nous le comparons à celui extrait du token. S'ils sont différents, nous générons une erreur et également si l'user n'est pas admin //
        return res.status(401).json({ error: "Unauthorized request" });
      } else {
        const commentario = {
          //Création d'un objet contenant le commentaire modifié par l'utilisateur contenue dans la requête//
          content: req.body.content,
          username: req.body.username,
          title: req.body.title,
        };
        Comment.update(commentario, {
          //Ensuite, nous créons le commentaire à envoyer à la base de donnée/
          where: {
            id: req.params.CommentId,
          } /*Et  modifions uniquement le comment qui a  l'id  égal que c'elle de l'url params*/,
        });
        res.send({
          message: "Comment was updated successfully.",
        });
    ...
    .then((commentModif) => {
      ...
      if (commentModif.userId !== req.auth.userId && isAdmin !== true) {
        ...
        Comment.destroy({
          ...
          where: {
            id: req.params.CommentId,
          } /*Et supprimons uniquement le comment qui a  l'id  égal que c'elle de l'url params*/,
        });
        res.send({
